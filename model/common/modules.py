...
class GuidanceModule(torch.nn.Module):
    ...
    def calculate_loss(model_output: tuple, labels: torch.Tensor) -> Tuple[torch.Tensor, Dict[str, Any]]:
        ...
        loss = torch.nn.functional.cross_entropy(final_logits, labels.squeeze().long())
        loss_info = {
            'guider': loss.item(),
        }
...
class FactorAnnealer:
    ...
    def step(self):
        decay_factor = self.decay_callback(self.current_step) # max(-float(current_step) * 1e5 + 1.0, 0.0),从1到0
        self.current_step += 1
